...
class Osc {
    ...
    var address_list_: CFArray? = [] as CFArray
    ...
    func GetAddress(host:CFHost) {
        ...
        for i in 0 ..< CFArrayGetCount(address_list_) {
            ...
            CFDataGetBytePtr(data).withMemoryRebound(to: sockaddr_in.self, capacity: 1, { address4 in
    ...
    func Send() {
        ...
        if (sa_family_ == UInt8(AF_INET)) {
            withUnsafePointer(to: &addr4_) { ptr in
    ...
    private func toByteArray<T>(value: T) -> [UInt8] {
        ...
        return withUnsafePointer(to: &value) {
            $0.withMemoryRebound(to: UInt8.self, capacity: 1, { addr in
                Array(UnsafeBufferPointer(start: addr, count: MemoryLayout<T>.size))
            })
